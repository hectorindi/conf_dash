...
class _AddMemberDelegateTypeState extends State<AddMemberDelegateType> {
  ...
  void initState() {
    ...
    if (widget.memberData[0]["error"] == null &&
        widget.memberData.isNotEmpty &&
        widget.memberData[0]["memberCategory"].isNotEmpty) {
      selectedMemberCategory =
          widget.memberData[0]["memberCategory"][0]["category"];
    }
  }
  ...
  void submitData() {
    ...
    memberService.value.addDelegateTypeToDatabase(delegateTypeController.text,
        _selectedItem!, rate, uid // Pass the uid as String
        );
    ...
    showNewDialog(
        context, Colors.green, "Member delegate type Added Successfully");
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButton<String>(
                  ...
                  items: widget.memberData[0]["memberCategory"]
...
DataRow recentUserDataRow(Map<String, dynamic> memberCategory,
  ...
  return DataRow(
    cells: [
      DataCell(
          Text(memberCategory['delegate_type'] ?? 'N/A')), // Category column
      DataCell(Text(memberCategory['createdAt']?.toString() ??
          'N/A')), // Created Date column
      DataCell(
        // Member Category column with dropdown
        DropdownButton<String>(
          value: selectedValue,
          items: memberCategory["memberCategory"]
              .map<DropdownMenuItem<String>>((category) {
            return DropdownMenuItem<String>(
              value: category['category'],
              child: Text(category['category']),
