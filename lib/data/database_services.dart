...
ValueNotifier<DatabaseServices> memberService =
    ValueNotifier(DatabaseServices());
...
class DatabaseServices {
  ...
  Future<bool?> addMemberDetailsToDatabase(
      UserCredential userCredential, MemberObject member) async {
  ...
  Future<bool?> getLasteMemeberRegID() async {
    try {
      CollectionReference ref =
          _firestore.collection(AppConstants.memberRegistrationLatestID);
  ...
  Future<bool?> addMemberCategoryToDatabase(
      String category, String status) async {
    try {
      CollectionReference ref =
          _firestore.collection(AppConstants.eventCollectionName);
      //log("Adding Member Category: $category with status: $status by user: $category");
      await ref
          .doc(AppConstants.memberCategoryDocName)
          .collection(AppConstants.memberCategoryColName)
          .add({
        'category': category,
        'status': status,
        'createdAt': FieldValue.serverTimestamp(),
  ...
  Future<bool?> addDelegateTypeToDatabase(
      String type, String status, double rate, String uid) async {
    try {
      CollectionReference ref =
          _firestore.collection(AppConstants.eventCollectionName);
      DocumentReference userRef = await ref
          .doc(AppConstants.memberCategoryDocName)
          .collection(AppConstants.memberCategoryColName)
          .doc(uid);
  ...
  Future<List<Map<String, dynamic>>> getSpecializationFromDatabase() async {
    try {
      CollectionReference ref = _firestore
          .collection(AppConstants.eventCollectionName)
          .doc(AppConstants.courseDocName)
          .collection(AppConstants.courseColName);
      //log("Fetching Member Category");
      QuerySnapshot querySnapshot = await ref.get();
      if (querySnapshot.docs.isNotEmpty) {
        List<Map<String, dynamic>> finalData = [];
  ...
  Future<List<Map<String, dynamic>>> getRegisterationDataFromDatabase() async {
    try {
      List<Map<String, dynamic>> memberCategory =
          await getMemberCategoryFromDatabase();
      List<Map<String, dynamic>> delegateTypes =
          await getMemberDelegateTypeFromDatabase();
      List<Map<String, dynamic>> specialization =
          await getSpecializationFromDatabase();
      //log("Fetching Member Category");
  ...
  Future<List<Map<String, dynamic>>> getMemberDelegateTypeFromDatabase() async {
    try {
      CollectionReference ref = _firestore
          .collection('aios_0925')
          .doc("delegate_category")
          .collection("ct_type");
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        List<Map<String, dynamic>> memberCategory =
            await getMemberCategoryFromDatabase();
        List<Map<String, dynamic>> finalList = querySnapshot.docs
            .map((doc) => doc.data() as Map<String, dynamic>)
            .toList();

        // Wait for all async operations to complete
        await Future.wait(finalList.map((action) async {
